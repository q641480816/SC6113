...
let web3bust, jsonProvider;

const getProvider = () => {
    try {
        // const web3 = new Web3.Web3("https://sepolia.infura.io/v3/8ee5212994f64e268c630ec7e48dff56");
        if (!web3bust) {
            web3bust = new Web3.Web3(`http${properties.ssl}://${properties.rpcURL}`);
        }
        return web3bust;
    } catch (err) {
        console.log(err);
...
const getContract = () => {
    ...
    return new web3.eth.Contract(abi, properties.votingContractAddress);
}
...
const getAvaliableUploader = () => {
    ...
    const mainCluster = () => {
        return new Promise((resolve, reject) => {
            hsetMassGet(uploaderTable, ...properties.uploaders)
                .then(uploader => {
                    let isFound = false;
                    for (let i = 0; i < uploader.length; i++) {
                        if (uploader[i] === 'true') {
                            ...
                            isFound = true;
                            hsetAdd(uploaderTable, properties.uploaders[i], false)
                                .then(res => resolve(properties.uploaders[i]))
...
const sendTransaction = async (method, params, eventNameForReturn) => {
    const uploaderTable = 'uploader';
    ...
    try {
        ...
        const wallet = new ethers.Wallet(uploaderPk, getJsonProvider());
        const callContract = new ethers.Contract(properties.votingContractAddress, abi, wallet);
        const txRes = await callContract[method](...params);
        console.log('Transaction hash:', txRes.hash);
        const receipt = await txRes.wait();
        console.log('Transaction was mined in block', receipt.blockNumber);
        hsetAdd(uploaderTable, uploaderPk, true);

        if (eventNameForReturn) {
            for (const log of receipt.logs) {
                try {
                    const parsedLog = callContract.interface.parseLog(log);
                    if (parsedLog.name === eventNameForReturn) {
                        console.log('Event:', parsedLog.name);
                        console.log(parsedLog.args)
                        return parsedLog.args;
                    }
                } catch (e) {
                    console.log('Log does not match this contract ABI');
                }
            }
        }
        ...
        //     to: properties.votingContractAddress,
        //     gas: estimatedGas * 2n,
