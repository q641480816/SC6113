...
const castVote = async (address, message, campaignId, signature = null) => {
    ...
    const campaignTable = properties.tables.campaignMap;
    ...
    try {
        ...
        if (!campaign) {
            const campaignOnChain = await getCampaign(campaignId);
            await redis.hsetAdd(campaignTable, campaignId, JSON.stringify(campaignOnChain));
        }else{
            campaign = JSON.parse(campaign);
        }
        if (!campaign['isActive']) throw Error(`Campaign [${campaignId}] ${campaign['name']} is not active!`);
...
const checkPendingCount = (campaignId) => {
    ...
    const primary = (campaignId) => {
        return new Promise(async (resolve, reject) => {
            ...
            try {
                const pendingCount = await redis.hsetCount(hkey);
                if (pendingCount >= properties.countThreshhold) {
                    logger.info(`CampaignId: ${campaignId} has hit the threshold, start compiling votes...`);
                    const allAddress = await redis.hsetGetKeys(hkey);
                    const addressToCompile = allAddress.slice(0, properties.countThreshhold);
...
const compileVote = async (campaignId) => {
    ...
    try {
        ...
        const result = await sendTransaction('uploadVoteBatch', [campaignId, batchCount, merkleRoot], 'UploadBatch');
        // const contract = getContract();
        // const events = await contract.getPastEvents('uploadVoteBatch', {
        //     filter: {},
...
module.exports = { compileVote, validateContract, startCampaign, endCampaign, castVote, setUploaderWithAddress, getCampaignResults, verifyMerkleProof, checkPendingCount };